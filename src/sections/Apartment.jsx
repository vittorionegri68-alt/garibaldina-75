...
export default function Apartment() {
  ...
  return (
    <section id="apartment" style={s.section} ref={sectionRef}>
      <div style={s.inner}>
        ...
        <div className="reveal" style={s.galleryWrap}>
          <div style={s.mainImg}>
            <img
              ...
              alt={`Garibaldina 75 - photo ${active + 1}`}
              style={s.mainImgEl}
              loading="lazy"
            />
          </div>
          <div style={s.thumbsCol}>
            {ROOM_IMAGES.map((img, i) => (
              <div key={i} style={s.thumb(i === active)} onClick={() => setActive(i)}>
                <img src={img} alt={`View ${i + 1}`} style={s.thumbImg} loading="lazy" />
              </div>
            ))}
          </div>
        </div>
